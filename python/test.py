...
from python.eventide import Parameters, Simulator, Scenario, IntervalCriterion, \
    IndexOffspringCriterion, DrawCollector, ActiveSetSizeCollector, TimeMatrix
from python.eventide_viz import plot_extinction
...
sim = Simulator(
    ...
    collectors=[
        ...
        draws := DrawCollector(),
        active_set_size := ActiveSetSizeCollector(datetime(2025, 4, 4)),
        time_matrix := TimeMatrix(datetime(2025, 4, 4)),
    ],
    num_trajectories=1_00_000_000,
    chunk_size=100_000,
    T_run=80,
    max_cases=100,
    max_workers=12,
...
print(sim.T_run)
# print(active_set_size.active_sets[:10])
# print(parantes_sizes.parent_sizes[:10])
plot_extinction(sim.start_date, time_matrix.cutoff_date, sim.T_run, analytical=(draws, active_set_size),
                ...
                empirical=time_matrix, dpi=600, figsize=(16, 8))
