...
from python.plots.style import days_between, _use_style
...
def band_from_target_and_share(
        M_hat: float, Y_hat: float, n_total: int,
        baseline_p: float, alpha: float, h_max: float, eps_share: float,
...
def proportional_observed_mass(
    ...
    by_day: Dict[Tuple[int, int, int], int] = {}
...
def build_cluster_windows(
        obs_points: Sequence[Tuple[datetime, int]], sigma_days: float, beta: float, neighbor_weight: float,
...
def plot_construction_side_by_side(
    ...
    cycle_cols = mpl.rcParams['axes.prop_cycle'].by_key().get('color', [sty.palette["BAND"]])
    COL_BAND = sty.palette["BAND"]
    COL_INTERP = sty.palette["INTERP"]
    COL_POINTS = sty.palette["POINTS"]
    COL_PAREN = sty.palette["PAREN"]
    COL_QUIET = sty.palette["QUIET"]
    ...
    times = [t for t, _ in obs]
    A, B = times[0], times[-1]
    n_total = int(sum(c for _, c in obs))
    ...
    def label_box(ax, xdt, y, text, valign):
        ax.text(xdt, y, text, ha="right", va=valign, fontsize=sty.annotation_size,
                bbox=dict(boxstyle="round,pad=0.25", fc="white", alpha=0.95, lw=0, ),
                zorder=6)
    ...
    def centered_band_rect(ax, a, b, center_y, L, U, k_index=None):
        ...
        ax.add_patch(Rectangle(
            ...
            height,
            facecolor=COL_BAND, edgecolor='none', alpha=0.45, linewidth=0.9, zorder=1.5,
        ))
        xr = b - timedelta(days=0.02 * max((b - a).total_seconds() / 86400.0, 1e-6))
        if k_index is None:
            label_box(ax, xr, y0 + 0.04 * height, rf"$L={L}$", "bottom")
            label_box(ax, xr, y0 + height - 0.04 * height, rf"$U={U}$", "top")
        else:
            label_box(ax, xr, y0 + 0.04 * height, rf"$L_{k_index}={L}$", "bottom")
            label_box(ax, xr, y0 + height - 0.04 * height, rf"$U_{k_index}={U}$", "top")

    def draw_parentheses(ax, tdt, y, a_left, b_right, color=COL_PAREN):
        width_days = max((b_right - a_left).total_seconds() / 86400.0, 1e-9)
        inset = timedelta(days=paren_inset_frac * width_days)
        a_in = a_left + inset
        b_in = b_right - inset
        ax.hlines(y, a_in, tdt, colors=color, linestyles="-", linewidth=0.9, alpha=0.30, zorder=2.3)
        ax.hlines(y, tdt, b_in, colors=color, linestyles="-", linewidth=0.9, alpha=0.30, zorder=2.3)
        ax.text(a_in, y, "(", ha="center", va="center", fontsize=sty.annotation_size, color=color, alpha=0.9,
                zorder=2.4)
        ax.text(b_in, y, ")", ha="center", va="center", fontsize=sty.annotation_size, color=color, alpha=0.9,
                zorder=2.4)
    ...
    for (a, b) in base_windows_cluster:
        M_hat = proportional_observed_mass(obs, A, B, a, b)
    ...
    for (a, b) in base_windows_seg:
        ...
        L, U, _, _ = band_from_target_and_share(M_hat, Y_hat, n_total, baseline_p, alpha, h_max, eps_share)
        seg_bands.append(WindowBand(a, b, L, U))

    fig, (axL, axR) = plt.subplots(1, 2, figsize=sty.fig_pair, sharey=True, dpi=sty.dpi, constrained_layout=True)

    # Left: Cluster–merging
    axL.set_title(r"Cluster–merging")
    for a, b in quiet_cluster:
        axL.axvspan(a, b, color=COL_QUIET, zorder=0.5, alpha=0.55)

    t_pts = [t for t, _ in obs]
    cum_pts = [N_obs_at(t) for t in t_pts]
    colors = [cycle_cols[0] for _ in t_pts]
    axL.scatter(t_pts, cum_pts, s=sty.dot_area, c=colors, zorder=2.8,
                edgecolor="white", linewidth=0.9, label=r"observed cumulative (points)")
    ...
    for k, w in enumerate(cluster_bands, start=1):
        ...
        center_y = (N_obs_at(w.start) + 0.5 * proportional_observed_mass(obs, A, B, w.start, w.end)) \
            if len(vals) == 0 else float(np.mean(vals))
    ...
    axL.legend(handles=[
        Patch(facecolor=COL_QUIET, edgecolor='none', label=r"quiet window", alpha=0.55),
        Patch(facecolor=COL_BAND, edgecolor='none', label=r"acceptance band $[L,U]$", alpha=0.45),
        Line2D([], [], color=COL_PAREN, lw=0.9, label=r"raw local window (parentheses)"),
        Line2D([], [], marker="o", lw=0, markersize=4, markerfacecolor=cycle_cols[0],
    ...
    axR.plot([A + timedelta(days=float(d)) for d in x_plot],
             ...
             color=COL_INTERP, lw=sty.lw_emp, zorder=2.0, label=r"interpolant")
    for (a, b) in base_windows_seg:
        if b <= a:
            continue
        xa, xb = days_between(A, a), days_between(A, b)
        ya, yb = float(f_interp(xa)), float(f_interp(xb))
        axR.plot([a, b], [ya, yb], color=COL_POINTS, lw=1.2, zorder=2.1, label=None)

    axR.scatter(t_pts, cum_pts, s=sty.dot_area, color='#BB4430', zorder=3.0, alpha=0.95,
                edgecolor="white", linewidth=0.9, label=r"observed cumulative (points)")
    ...
    if savepath:
        plt.savefig(savepath, dpi=sty.dpi)
    plt.show()
